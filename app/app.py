...
# Mock mode settings
ABB_MOCK_MODE = os.getenv("ABB_MOCK_MODE", "false").lower() == "true"
ABB_SCHEME = os.getenv("ABB_SCHEME", "http" if ABB_MOCK_MODE else "https")

PAGE_LIMIT = int(os.getenv("PAGE_LIMIT", 5))

# Debug settings
DEBUG_RESPONSES = os.getenv("DEBUG_RESPONSES", "false").lower() == "true"
...
print(f"ABB_MOCK_MODE: {ABB_MOCK_MODE}")
print(f"ABB_SCHEME: {ABB_SCHEME}")
...
print(f"DEBUG_RESPONSES: {DEBUG_RESPONSES}")
...
def save_debug_response(response, request_type, metadata):
    """
    Saves HTTP response and metadata to debug directory for mocking purposes.

    Args:
        response: The requests.Response object
        request_type: Type of request (e.g., "search" or "detail")
        metadata: Dictionary with additional metadata (query, page, etc.)
    """
    ...
    try:
        ...
        # Create safe filename based on request type and metadata
        if request_type == "search":
            query = metadata.get("query", "unknown").replace(" ", "_")[:50]
            page = metadata.get("page", 0)
            base_filename = f"{timestamp}_search_{query}_page{page}"
        elif request_type == "detail":
            # Extract book slug from URL for filename
            url_parts = metadata.get("url", "").split("/")
            slug = url_parts[-1] if url_parts else "unknown"
            slug = slug[:50]  # Limit length
            base_filename = f"{timestamp}_detail_{slug}"
        else:
            base_filename = f"{timestamp}_{request_type}"

        # Sanitize filename
        base_filename = re.sub(r'[<>:"/\\|?*]', '_', base_filename)

        # Save HTML response
        html_path = os.path.join(DEBUG_DIR, f"{base_filename}.html")
        ...
        metadata_path = os.path.join(DEBUG_DIR, f"{base_filename}_metadata.json")
        with open(metadata_path, "w", encoding="utf-8") as f:
            json.dump(metadata_content, f, indent=2)

        print(f"[DEBUG] Saved response to {html_path}")
        print(f"[DEBUG] Saved metadata to {metadata_path}")

    except Exception as e:
        print(f"[WARNING] Failed to save debug response: {e}")
...
def search_audiobookbay(query, max_pages=PAGE_LIMIT):
    ...
    Returns:
        tuple: (list of results, error_message or None)
    """
    headers = {
    ...
    print(f"Searching for '{query}' on {ABB_SCHEME}://{ABB_HOSTNAME}...")

    for page in range(1, max_pages + 1):
        url = f"{ABB_SCHEME}://{ABB_HOSTNAME}/page/{page}/?s={query.lower().replace(' ', '+')}"
        try:
            ...
            save_debug_response(response, "search", {
                "query": query,
                "page": page,
                "max_pages": max_pages,
                "url": url
            ...
            if "507" in error_msg or "Insufficient Storage" in error_msg:
                error_message = "AudiobookBay is currently experiencing server issues (507 error). This may be due to rate limiting or server maintenance. Please try again later."
            elif "timeout" in error_msg.lower() or "timed out" in error_msg.lower():
                error_message = "Request timed out. AudiobookBay may be slow or unreachable. Please try again."
            elif "404" in error_msg:
                error_message = "The requested page was not found on AudiobookBay."
            elif "429" in error_msg or "Too Many Requests" in error_msg:
                error_message = "Rate limit exceeded. Please wait a few minutes before searching again."
            else:
                error_message = f"Failed to connect to AudiobookBay: {error_msg}"

            # Only break on first page error, otherwise just stop pagination
            if page == 1:
                break
            break
        ...
        for post in posts:
            try:
                ...
                link = f"{ABB_SCHEME}://{ABB_HOSTNAME}{title_element['href']}"
...
def extract_magnet_link(details_url):
    ...
    try:
        ...
        # Save debug response if enabled
        save_debug_response(response, "detail", {
            "url": details_url,
    ...
    try:
        if request.method == "POST":  # Form submitted
            ...
            if query:  # Only search if the query is not empty
                books, search_error = search_audiobookbay(query)
                if search_error:
                    error = search_error
        return render_template("search.html", books=books, query=query, error=error)
    except Exception as e:
...
def api_search():
    ...
    try:
        ...
        # If there's a search error but we got some results, include both
        response = {
            "query": query,
...
def api_download():
    """
    API endpoint to download an audiobook to the configured torrent client.
    Accepts JSON payload with 'link' and 'title' fields from /api/search response.
    """
    data = request.json
    if not data:
    ...
    if not details_url:
        return jsonify({"error": "Missing required field: 'link'"}), 400
    if not title:
    ...
    try:
        ...
        if not magnet_link:
            return jsonify({"error": "Failed to extract magnet link from the provided URL"}), 500

        # Create save path based on sanitized title
        save_path = f"{SAVE_PATH_BASE}/{sanitize_title(title)}"

        # Add torrent to configured client
        if DOWNLOAD_CLIENT == "qbittorrent":
            ...
            return jsonify({"error": f"Unsupported download client: {DOWNLOAD_CLIENT}"}), 500
...
def status():
    try:
        if DOWNLOAD_CLIENT == "transmission":
            ...
            # Obfuscate password
            if DL_PASSWORD:
                obfuscated_password = "*" * min(len(DL_PASSWORD), 12)
            else:
                obfuscated_password = "No Password Set"

            config_data = {
                "abb_url": f"{ABB_SCHEME}://{ABB_HOSTNAME}",
                "abb_hostname": ABB_HOSTNAME,
                "abb_scheme": ABB_SCHEME,
                "mock_mode": ABB_MOCK_MODE,
                "page_limit": PAGE_LIMIT,
                "debug_mode": DEBUG_RESPONSES,
                "dl_client": DOWNLOAD_CLIENT,
                "dl_url": DL_URL,
                "dl_username": DL_USERNAME,
                "dl_password_display": obfuscated_password,
                "dl_category": DL_CATEGORY,
                "save_path": SAVE_PATH_BASE,
            }
            return render_template("status.html", torrents=torrent_list, config=config_data)
